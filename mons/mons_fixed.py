...
class PerformanceMonitor:
    def __init__(self, root):
        ...
        # Настройка данных графиков
        # Создание очередей для хранения последних
        ...
        # Вызов метода для настройки пользовательского интерфейса
        self.setup_ui()
    ...
    def draw_graph(self, canvas, data, color, title, max_value=100):
        ...
        for i in range(0, 101, 20):  # Линии каждые 20%
            ...
            canvas.create_line(0, y, width, y, fill='lightgrey', dash=(2, 2))
        ...
        if len(points) > 1:  # Проверка, что есть хотя бы 2 точки
    ...
    def get_performance_data(self):
        ...
        # Диск (берем первый диск)
        disk = psutil.disk_usage('/')
        disk_percent = disk.percent

        # Сеть (упрощенная версия)
        ...
        network_usage = (net_after - net_before) / 1024  # KB/s

        return cpu_percent, memory_percent, disk_percent, network_usage

    def update_data(self):
        cpu, memory, disk, network = self.get_performance_data()
        ...
        self.memory_label.config(text=f'Память: {memory:.1f}%')
        self.disk_label.config(text=f'Диск: {disk:.1f}%')
        self.network_label.config(text=f'Сеть: {network:.1f} KB/s')

        # Перерисовываем графики
        self.draw_graph(self.cpu_canvas, self.cpu_data, 'red', 'Использование процессора')
        self.draw_graph(self.memory_canvas, self.memory_data, 'blue', 'Использование памяти')
        self.draw_graph(self.disk_canvas, self.disk_data, 'green', 'Использование диска')
        self.draw_graph(self.network_canvas, self.network_data, 'orange', 'Сетевая активность', max_value=1000)
...
def main():
    ...
    try:
        ...
        print('Установите его с помощью pip install psutil')
        return
